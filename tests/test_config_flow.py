...
async def test_existing(hass, enable_custom_integrations):
    ...
    host = "1.2.3.4"
    MockConfigEntry(
        domain=DOMAIN, data={dynalite.CONF_HOST: host}
    ).add_to_hass(hass)
    with patch(
        "custom_components.dynalite2.bridge.DynaliteDevices.async_setup",
        return_value=True,
...
async def test_existing_update(hass, enable_custom_integrations):
    ...
    with patch(
        ...
        "custom_components.dynalite2.bridge.DynaliteDevices.configure"
    ) as mock_configure:
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
        mock_configure.assert_called_once()
        assert mock_configure.mock_calls[0][1][0]["port"] == port1
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
            context={"source": config_entries.SOURCE_IMPORT},
        ...
        assert mock_configure.mock_calls[1][1][0]["port"] == port2
    assert result["type"] == "abort"
    assert result["reason"] == "already_configured"
...
async def test_two_entries(hass, enable_custom_integrations):
    ...
    host2 = "5.6.7.8"
    MockConfigEntry(
        domain=DOMAIN, data={dynalite.CONF_HOST: host1}
    ).add_to_hass(hass)
    with patch(
        "custom_components.dynalite2.bridge.DynaliteDevices.async_setup",
        return_value=True,
